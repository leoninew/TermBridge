  ...
  >
    ...
    >
      <div class="mx-auto max-w-3xl text-center">
        <h1
          class="text-3xl font-semibold tracking-tight text-slate-950 md:text-4xl dark:text-slate-100"
        >
        ...
        <p
          v-else-if="environmentStore.error"
          class="mt-4 border-l border-red-300 bg-red-50/70 px-4 py-3 text-red-700 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300"
        >
      ...
      <div class="grid gap-4 md:grid-cols-3">
        ...
        >
          <span
            class="inline-flex h-11 w-11 items-center justify-center rounded-lg bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300"
          >
            <component :is="option.icon" class="h-5 w-5" />
          </span>
          <span>
            <span class="block text-lg font-semibold text-slate-950 dark:text-slate-100">{{
              t(option.titleKey)
            }}</span>
            <span class="mt-2 block leading-6 text-slate-600 dark:text-slate-400">{{
              t(option.descriptionKey)
            }}</span>
          </span>
          <span
            class="mt-auto inline-flex items-center gap-1 font-medium text-blue-700 dark:text-blue-400"
          >
      ...
      <div
        class="mx-auto flex flex-wrap items-center justify-center gap-3 text-slate-600 dark:text-slate-400"
      >
