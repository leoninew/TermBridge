# Frontend font size consistency verification
...
- 非标题类显式小字号统一到 `text-sm`：
  ...
  - `frontend/src/components/EnvironmentManagement.vue`
- 补齐隐式默认字号问题：
  - `frontend/src/App.vue` 的 `<main>` 增加 `text-sm`，作为普通文本兜底，避免未显式字号的内容回退到浏览器默认 16px。
  - `frontend/src/components/SessionList.vue` 的 loading/error/empty 文本补 `text-sm`，其中 “暂无会话，先创建一个。” 现在是 `text-sm`。
...
- [x] `frontend/src` 中未发现普通 UI 文本继续使用 `text-xs`、`text-base`、`text-xl`、`text-2xl` 或任意值字号类。
...
- `npm --prefix frontend run lint`
  - 结果：通过。
- 搜索检查：`frontend/src` 中未匹配 `text-(xs|base|xl|2xl|\[[^\]]+\])`。
