...
- [x] `create_app()` 初始化 logging，确保 `uvicorn cc_ttyd.main:app --reload` / `just dev-backend` 不绕过请求日志配置。
...
本任务相关日志功能目标已达成；`just dev-backend` / Uvicorn CLI reload 入口也会执行 logging 初始化，请求级别日志应能进入统一 console handler。
