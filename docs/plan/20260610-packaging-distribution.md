...
3. 新增可选参数：
   - `--frontend-dir PATH`：显式指定构建后的前端目录。
   - `--no-frontend`：开发或调试时强制 API-only。
4. 默认行为：启用前端服务；如果包内或指定目录没有 `index.html`，后端自动退化为 API-only。
5. 保留 `app = create_app()`，使现有 reload 字符串入口继续可用。
6. 当用户传入 `--frontend-dir` 或 `--no-frontend` 时，运行动态创建的 app 对象；否则沿用 `uvicorn.run("termbridge.main:app", ...)`。
...
1. `make wheel` 构建前端并同步到 `src/termbridge/static/`
2. 同一个 `make wheel` 目标随后运行 `uv build`
...
1. frontend build stage：
   - 使用 Node 镜像。
   - 安装 frontend 依赖。
   - 运行 `yarn --cwd frontend build`。
2. backend runtime stage：
   - 使用 Python slim 镜像。
   - 复制 Python 项目文件。
   - 从 frontend stage 复制 `frontend/dist` 到 `src/termbridge/static`。
   - `pip install .`。
   - `EXPOSE 9008`。
...
1. 使用 `tmp_path` 创建临时 frontend 目录，包含：
   - `index.html`
   - 一个静态 asset 文件
2. 验证：
   - `/` 返回 `index.html`
...
3. 说明 `termbridge` 默认服务前端；`--no-frontend` 可用于 API-only 调试。
...
3. Wheel：
   - 将 `frontend/dist/*` 复制到 `src/termbridge/static/`
   - `uv build`
