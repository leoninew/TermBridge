...
> [!IMPORTANT]
> TermBridge 当前版本**还没有内置**用户认证、授权、HTTPS、多用户隔离或托管部署加固。受管理的 `ttyd` session 默认绑定到 `127.0.0.1` 并使用本地 Basic Authentication，但这只是本地加固，不是部署安全边界。如果通过公网、移动端、tunnel 或反向代理访问，请在 TermBridge 外层配置合适的访问控制、TLS 和隔离措施。不要暴露未保护的实例。
